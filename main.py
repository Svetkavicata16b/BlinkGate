...
#         self.prev_time = time.time()
#
#     def to_text(self, morse_code_letter, separator):
#         try:
#             self.text.configure(state="normal")
#             self.text.insert(tk.INSERT, morse_code_english_dict[morse_code_letter])
#         except KeyError:
#             self.text.insert("insert", "#")
#         finally:
#             self.text.insert("insert", separator)
#             self.text.configure(state="disabled")
#
#
#     def to_morse_code(self, eyes):
...
#             if current_time - self.prev_time > 3:
#                 self.morse_code.configure(state="normal")
#                 self.morse_code.insert("insert", " / ")
#                 self.morse_code.configure(state="disabled")
#                 self.to_text(self.last_letter, " ")
#                 self.last_letter = ""
#             elif current_time - self.prev_time > 1:
#                 self.morse_code.configure(state="normal")
#                 self.morse_code.insert("insert", " ")
...
#     if event.keysym == "Left":
#         screen.to_morse_code([True, False])
#         screen.to_morse_code([False, False])
#     elif event.keysym == "Right":
#         screen.to_morse_code([False, True])
#         screen.to_morse_code([False, False])
#
# screen.screen.bind_all("<Left>", key_handler)
# screen.screen.bind_all("<Right>", key_handler)
#
# print(model.to_morse_code([True, False]))
# print(model.to_morse_code([False, False]))
#
